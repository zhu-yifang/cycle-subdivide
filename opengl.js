...
function glInit() {
    ...
    _GL.ambientColor   = [0.5, 0.5, 0.7, 1.0];
    _GL.lighting       = false;
    _GL.light0         = false;
...
function glutTimerFunc(msecs, timerHandler, value) {
    _GL.timerFunc = timerHandler;
    _GL.timerDelta = msecs;
    _GL.timerValue = value;
}
...
function glutMainLoop() {
    const loop = ((time_in_msecs) => {
        //
        if (_GL.timerFunc != null) {
            if (time_in_msecs - _GL.lastTime > _GL.timerDelta) {
                _GL.lastTime = time_in_msecs;
            }
            _GL.timerFunc(_GL.timerValue,time_in_msecs);
        }
	    _GL.displayFunc();
        //
	    glutMainLoop();
...
function glModColor(name, index, newColor) {
    console.assert(_GL.recordings.has(name));
    const recording = _GL.recordings.get(name);
    //
    // newColor = {r:0.4, g:0.3, b:0.2};
    recording.colors[12*index]    = newColor.r;
    recording.colors[12*index+ 1] = newColor.g;
    recording.colors[12*index+ 2] = newColor.b;
    recording.colors[12*index+ 3] = 1.0;
    recording.colors[12*index+ 4] = newColor.r;
    recording.colors[12*index+ 5] = newColor.g;
    recording.colors[12*index+ 6] = newColor.b;
    recording.colors[12*index+ 7] = 1.0;
    recording.colors[12*index+ 8] = newColor.r;
    recording.colors[12*index+ 9] = newColor.g;
    recording.colors[12*index+10] = newColor.b;
    recording.colors[12*index+11] = 1.0;
    //
	gl.bindBuffer(gl.ARRAY_BUFFER, recording.colorsBuffer);
	const colors = new Float32Array(recording.colors);
...
function glEnd() {
    ...
    if (_GL.saveColors) {
        ...
        _GL.recording.colors = _GL.colors;
...
function gluTargetTo(eye, center, up) {
    const m = mat4.create();
    const e = vec3.fromValues(eye.x, eye.y, eye.z);
    const c = vec3.fromValues(center.x, center.y, center.z);
    const u = vec3.fromValues(up.dx, up.dy, up.dz);
    mat4.targetTo(m, e, c, u);
    mat4.multiply(_GL.matrix[_GL.matrixMode],
		          _GL.matrix[_GL.matrixMode],m);
}
